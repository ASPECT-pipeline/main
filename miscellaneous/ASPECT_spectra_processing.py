...
def process_ESA_meteorite_directory(
	...
	'''
	This function is the main processing function to derive the spectra.
	First it reads some metadata, then it reads the spectra values,
	makes calculations, and plots the results.

	Take note: At the moment the binning functionality is hardcoded, and it is defined below.
	'''
	...
	'''
	This section defines the wavelengths based on the image setpoints in metadata
	A recalibration of setpoint to wavelength is done separately in a different file.
	This code uses the functions that are defined in the wavelength recalibration file.
	For new data, the wavelength recalibration will need to be redone.
	'''
	acquisition_wavelengths = []
	task_file_name = channel.lower() + 'TaskFile'
	with open(config_path, 'r') as file:
	...
	for sequence in task_file_sequences:
		...
		if channel.lower() == 'vis':
			if order == 'lo':
				wl = vis_lo_setpoint_to_wavelength(sequence[1], vis_edge_correction) # This function is defined in another file, and imported here.
			elif order == 'ho':
	...
	# This lists the individual images (binary files)
	...
	'''
	Here we retrieve the calibration values from the calibration images.
	These will be used to calculate reflectance.
	'''
	...
	# This section retrieves the raw Pixel Values for each coordinate and plots them
	'''
	Take note: The binning is hardcoded here as 2x2 binning. There is a better implementation
	of the binning in the reflectance section below this section. This is kind of a deprecated
	section because we are mainly interested the reflectance values.
	'''
	...
	# This section retrieves the reflectances for each coordinate and plots them
	'''
	The binning has a separate function that you can use to define the binning size.
	There is also a simple "Binning" variable that you can set to some integer value.
	For example, if you set binning = 3, then it will do 3x3 binning. If you set binning = None,
	then it will just use the single pixel value.
	'''
	binning = 3 # Set meteorite spectra binning size, example: 3 for 3x3 or None for single pixel
	...
	if not pixel_coordinates:
		...
		for i, (x_coord, y_coord) in enumerate(pixel_coordinates):
			...
			for wl, bin_file in zip(filtered_wls, filtered_binfiles):
				...
				if type(rectangle_coordinates) == tuple:
					pixel_reflectance = reflectance(average_value, rectangle_averages[os.path.join(actual_data_directory_naming_key, bin_file)]) # the main reflectance value
					binning_pixel_reflectance = reflectance(average_pix, rectangle_averages[os.path.join(actual_data_directory_naming_key, bin_file)]) # Old and unused (I think)
				else:
					pixel_reflectance = reflectance(average_value, calibration_pixel_values[os.path.join(actual_data_directory_naming_key, bin_file)][i]) # the main reflectance value
					binning_pixel_reflectance = reflectance(average_pix, calibration_pixel_values[os.path.join(actual_data_directory_naming_key, bin_file)][i]) # Old and unused (I think)
				binning_pixel_reflectances.append(binning_pixel_reflectance)
				pixel_reflectances.append(pixel_reflectance)
	...
	'''
	The binning here is done in the old way where it is hardcoded to 2x2 binning.
	You can copy how reflectance section does the binning if you want to change it.

	Currently the signal to noise ratio plots are based on the 2x2 binning but
	you can change this if needed.

	The plot is defined here at the beginning and the individual subplots are filled
	one by one.
	'''
	fig, axes = plt.subplots(num_coordinates, 1, figsize=(7, 2*num_coordinates), sharex=True)
	if num_coordinates == 1:
	...
	if not pixel_coordinates:# This is for calculating the signal to noise ratio of the averaged area
		...
		ax.plot(range(len(x_labels)), binning_signal_to_noise_ratios, c='blue') # You can change this to signal_to_noise_ratios if you want the single pixel values
		...
		for i, pixel_coordinate in enumerate(pixel_coordinates):# This is for calculating the signal to noise ratio of the individual coordinates
			signal_to_noise_ratios = []
			binning_signal_to_noise_ratios = []
			for j, bin_file in enumerate(filtered_binfiles):
				file_path = os.path.join(actual_data_directory, bin_file)
				with open(file_path, 'rb') as file:
					...
					img_array = np.frombuffer(bin_data, dtype=np.uint16)
					img_array = img_array.reshape((height, width))
					# The following 5 lines are the hardcoded 2x2 binning I mentioned before
					pixel1 = img_array[pixel_coordinate[1], pixel_coordinate[0]]
					pixel2 = img_array[pixel_coordinate[1], pixel_coordinate[0]+1]
					pixel3 = img_array[pixel_coordinate[1]+1, pixel_coordinate[0]]
					pixel4 = img_array[pixel_coordinate[1]+1, pixel_coordinate[0]+1]
	...
	if type(rectangle_coordinates) == tuple:
		return rectangle_averages, all_coordinates_pixel_values, all_coordinates_pixel_reflectances, None, x_labels, num_coordinates, min_y_pixel_value, max_y_pixel_value, min_y_reflectance, max_y_reflectance, None, None
	else:
		return calibration_pixel_values, all_coordinates_pixel_values, all_coordinates_pixel_reflectances, None, x_labels, num_coordinates, min_y_pixel_value, max_y_pixel_value, min_y_reflectance, max_y_reflectance, None, None
...
def process_grouped_directories_with_specified_metadata(
	...
	for vis_metadata_directory, nir1_metadata_directory, nir2_metadata_directory, vis_actual_data_directory, nir1_actual_data_directory, nir2_actual_data_directory, vis_matching_dark_directory, nir1_matching_dark_directory, nir2_matching_dark_directory, output_directory, vis_pixel_coordinate, nir1_pixel_coordinate, nir2_pixel_coordinate, vis_rectangle_coordinate, nir1_rectangle_coordinate, nir2_rectangle_coordinate, vis_averaged_spectra_square_coordinate, nir1_averaged_spectra_square_coordinate, nir2_averaged_spectra_square_coordinate, vis_calibration_data_directory, nir1_calibration_data_directory, nir2_calibration_data_directory, nir_spectra_shifting, vis_offset in zip(vis_metadata_directories, nir1_metadata_directories, nir2_metadata_directories, vis_actual_data_directories, nir1_actual_data_directories, nir2_actual_data_directories, vis_matching_dark_directories, nir1_matching_dark_directories, nir2_matching_dark_directories, output_directories, vis_pixel_coordinates, nir1_pixel_coordinates, nir2_pixel_coordinates, vis_rectangle_coordinates, nir1_rectangle_coordinates, nir2_rectangle_coordinates, vis_averaged_spectra_square_coordinates, nir1_averaged_spectra_square_coordinates, nir2_averaged_spectra_square_coordinates, vis_calibration_data_directories, nir1_calibration_data_directories, nir2_calibration_data_directories, nir_spectra_shifting_list, vis_offsets):
		...
		# This section combines the spectra from all channels
		combined_all_coordinates_values = [] # [coordinate[spectra_with_all_channels]]]
...
'''
averaged_spectra = False
uniform_spots = True

'''
This is the beginning of configuration lists for data processing.
Because the VIS, NIR1, and NIR2 are effectively separate acquisition,
configuration lists are made separately for each channel. The channels
are then matched in the processing function.
...
# Define the VIS metadata directories for each acquisition
if averaged_spectra:
...
# Define the NIR1 metadata directories for each acquisition
if averaged_spectra:
...
# Define the NIR2 metadata directories for each acquisition
if averaged_spectra:
...
# Define the VIS actual data directories for each acquisition
'''
These are the dark-subtracted versions of the meteorite images.
For a reminder, the dark-subtracted meteorites are derived by
subtracting the dark averages from the raw meteorite images
in the following way:
raw meteorite images - corresponding dark average = dark-subtracted meteorite images
'''
if averaged_spectra:
...
# Define the NIR1 actual data directories for each acquisition
if averaged_spectra:
	nir1_actual_data_directories = [
...
# Define the NIR2 actual data directories for each acquisition
if averaged_spectra:
	nir2_actual_data_directories = [
		# '/home/sysa/HERA/test_data/test_outputs/ESA meteorites dark subtracted (16-bit signed, NIR cropped to 636x506)/2024-08-29_14-43-36_nir2_Kilabo_nir2_25000', # Saturated
...
# Select the VIS binary files that you want to use for visualization of pixel locations
# (This does not affect the processing itself)
if averaged_spectra:
...
# Select the NIR1 binary files that you want to use for visualization of pixel locations
if averaged_spectra:
...
# Select the NIR2 binary files that you want to use for visualization of pixel locations
if averaged_spectra:
...
# Define the VIS matching dark directories for each acquisition
'''
These are only used to derive the signal-to-noise ratio (SNR) estimates for each acquisition.
You will need an additional excel file that contains the standard deviations. A correct standard deviation
is found based on these dark average names.
'''
if averaged_spectra:
...
# Define the NIR1 matching dark directories for each acquisition
if averaged_spectra:
...
# Define the NIR2 matching dark directories for each acquisition
if averaged_spectra:
	nir2_matching_dark_directories = [
...
# The output directories are self-explanatory. Thats where the processed data will be saved.
if averaged_spectra:
	output_directories = [
...
# Empty lists. These will be filled automatically later.
vis_pixel_coordinates = []
nir1_pixel_coordinates = []
nir2_pixel_coordinates = []

'''
The name rectangle_coordinates is misleading. Here we define how the calibration images are
applied to the data.

If the type of the items in the list is None, the same coordinates from
the meteorite images are applied to the calibration images.
If the type of the items in the list is a list, the coordinates are expected
to describe the positions of individual pixels in the calibration images,
as opposed to an averaged area.
If the type of the items in the list is a tuple, the coordinates are expected to
represent the corners of an area that will be averaged to get the calibration value.

For ESA Meteorites, calibration squares are taken from "ESA 20" images.

Some of the calibration standards are not at the positions of the selected
coordinates. In those cases the calibration coordinates have to be
selected manually here. There is an example below (NIR2), where other list indices are None,
and one has a list of manually selected coordinates.
'''
if averaged_spectra:
	vis_rectangle_coordinates = [
...
# Calibration coordinates for NIR1
if averaged_spectra:
	nir1_rectangle_coordinates = [
...
# Calibration coordinates for NIR2
if averaged_spectra:
	nir2_rectangle_coordinates = [
...
if uniform_spots:
	nir2_rectangle_coordinates = [
		...
		[(451,304), (374,332), (244,344), (118,288)], # This is an example where the calibration pixels have been selected manually because the calibration standard (in the calibration image) was at the different position than the selected coordinates.
		# None,
		None,
		None,
		None,
...
# Helper lines for defining the spectra coordinates accurately across different channels
'''
Example:
[((642,503),(334,675)), ((642,503),(240,589))]

this is a list that contains two lines (tuples).
The tuples contain the coordinates for the start and end points
of the lines.
'''
if averaged_spectra:
...
# Pixel coordinates based on the helper lines above.
'''
These represent percentages across the helper lines, which are then used to
select the pixels that we want to derive a spectra from.

Example:
One helper line with 8 pixels: [[0.0001, 0.2, 0.25, 0.4, 0.5, 0.555, 0.83, 0.9]]
Two helper lines with 4 pixels each: [[0.1, 0.25, 0.5, 0.83], [0.2, 0.4, 0.555, 0.8]]
'''
if averaged_spectra:
...
# NIR1 pixel coordinates based on the helper lines above.
if averaged_spectra:
...
# NIR2 pixel coordinates based on the helper lines above.
if averaged_spectra:
...
# VIS coordinates for averaged rectangle coordinates
'''
This is only for averaged spectra. If you want to generate individual
pixel or 3x3 binning spectra, you can just make a list of None values.
'''
if averaged_spectra:
...
# NIR1 coordinates for averaged rectangle coordinates
if averaged_spectra:
...
# NIR2 coordinates for averaged rectangle coordinates
if averaged_spectra:
...
# For ESA meterorites we used "ESA 20" files
if averaged_spectra:
...
# File paths to the NIR1 calibration images
if averaged_spectra:
...
# File paths to the NIR2 calibration images
if averaged_spectra:
...
'''
These following configurations are for the post-processing, or smoothing of the
spectra to align the VIS, NIR1, and NIR2 spectra together.

This specifies which NIR channel (1 or 2) is to be shifted to match the other one.
Options are None, 'nir1', or 'nir2'
The shifting for NIR channels is done automatically
'''
# Which channel to shift, if any
if averaged_spectra:
...
'''
This is a manually determined VIS offset
Baseline is 0, positive values will shift the VIS spectra up
Negative values will shift the VIS spectra down
'''
if averaged_spectra:
	vis_offsets = [
		0, # Kilabo nir2 15000
...
# This populates the empty lists above with actual pixel coordinates
