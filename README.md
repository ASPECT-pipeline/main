...
![calibration_pipeline_diagram](documents/ASPECT_data_pipeline_flow_diagram.png)
...
`software` = Software name for the pipeline
`missphase` = Hera Mission Phase ID
`observph` = Hera Observation ID
`target` = Observation target (SPICE format)
`object` Observed object
`sc_clock_seconds` = Spacecrafk clock count
`sc_clock_offset` = Spacecraft clock offset 

`spice_mk` = Absolute path to the SPICE metakernel used for retrieving spice data for FITS headers

`pipeline` = A string describing to pipeline which levels you want to excecute, separated with '-'. e.g. a string '1-2-3' will execute all levels for the input data, while a string '1' will execute only the levels 0 and 1. If skipping a level, make sure the input directory will contain fits file with expected ending. ('1B.fits' for level 2 and  '2B.fits' for level 3).
...
MGM analysis and neural network to composition and taxonomy analysis are found from ASPECT_calibration_pipeline/level_3
